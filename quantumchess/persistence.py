...
def to_dict(qb: QuantumBoard, config: GameConfig, rng: random.Random,
    ...
    return {
        ...
        "config": {
            ...
            "split_stay_enabled": config.split_stay_enabled,
            "mass_movement": config.mass_movement,
            "mass_split": config.mass_split,
            "mass_all_must_act": config.mass_all_must_act,
            "seed": config.seed,
            "theme": config.theme,
...
def from_dict(data: dict[str, Any]) -> tuple[QuantumBoard, GameConfig, random.Random, str, list[str]]:
    ...
    config = GameConfig(
        ...
        split_stay_enabled=cfg_data.get("split_stay_enabled", default_config.split_stay_enabled),
        mass_movement=cfg_data.get("mass_movement", default_config.mass_movement),
        mass_split=cfg_data.get("mass_split", default_config.mass_split),
        mass_all_must_act=cfg_data.get("mass_all_must_act", default_config.mass_all_must_act),
        seed=cfg_data["seed"],
...
def load_last_settings(path) -> dict[str, Any]:
    ...
    return {
        ...
        "split_stay_enabled": data.get("split_stay_enabled", default.split_stay_enabled),
        "mass_movement": data.get("mass_movement", default.mass_movement),
        "mass_split": data.get("mass_split", default.mass_split),
        "mass_all_must_act": data.get("mass_all_must_act", default.mass_all_must_act),
